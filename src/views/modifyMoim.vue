<template>
  ...
  >
    <div id="main-text">
      <b id="main-text-highlight">모임 정보 수정하기</b>
    </div>
...
import { MoimDetail, modifyMoim } from "@/services/moim";
export default {
  data() {
    return {
      moimDetail: null,
  ...
  methods: {
    ...
    async clickModifyButton() {
      ...
      console.log(data);
      await modifyMoim(data);
      alert("모임 정보 수정 완료");
      this.$router.replace("/moimteam");
    },
    async setMoimData(id) {
      let moimData = await MoimDetail(id);
      this.moimDetail = moimData.data;
      console.log(this.moimDetail);
      // 모임방 이름
      this.moimtitle = this.moimDetail.title;
      //모임방 카테고리
      var findParent = false;
      for (var i = 0; i < this.parentCategory_options.length; i++) {
        var parentCategory = this.parentCategory_options[i];
        let childCategoryList = await getChildCategory(
          parentCategory.categoryId
        );
        for (var j = 0; j < childCategoryList.data.length; j++) {
          if (
            childCategoryList.data[j].categoryId == this.moimDetail.categoryId
          ) {
            findParent = true;
            console.log(parentCategory);
            console.log(childCategoryList.data[j]);
            break;
          }
        }
        if (findParent == true) {
          this.selected_category.parentCategory = parentCategory.categoryId;
          let child = await getChildCategory(
            this.selected_category.parentCategory
          );
          this.selected_category.childCategory_options = child.data;
          this.selected_category.childCategory = this.moimDetail.categoryId;
          break;
        }
      }
      // 활동지역
      this.region1_selected = this.moimDetail.sido;
      this.UpdateLocation(1,this.region1_selected);
      this.region2_selected = this.moimDetail.sigungu;
      this.UpdateLocation(2,this.region2_selected);
      this.region3_selected = this.moimDetail.dong;
      // 소개
      this.content = this.moimDetail.content;
      //정원
      this.maxMember = this.moimDetail.maxMember;
      //가입방식
      this.isFreeEnter_selected= this.moimDetail.isFreeEnter;
      //공개여부
      this.isPublish_selected = this.moimDetail.isPublish;
    },
  },
  created() {
